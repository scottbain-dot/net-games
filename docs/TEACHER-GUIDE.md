...
- **BackPage** — optional page 2 of every printed student log, so a teacher can add their own material without touching the app. One row per line, in order: `Sport` (blank = every sport), `Line`. A line starting `# ` prints as a heading, `- ` as a bullet, `___` as a ruled writing line, an empty line as a gap. Leave the tab empty for a single-sided log.
