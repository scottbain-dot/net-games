...
|---|---|---|
| **Before L1** | — | **Print → Unit plan** for each sport and hand it to the teacher taking that group to check. Then **Print → One sheet per student**: every sheet carries the three tests, a blank focus-skill line with retest boxes, all three skills' drill steps, and a nine-lesson strip with personal-skills tick boxes, so one sheet lasts the whole unit. |
...
| **L2–L4** | Works the drill steps on the paper log; a partner initials each step; you sign it; ticks the three personal skills for the lesson | Circulates; **signs the step** to let a student move on. **No app.** |
