(function() {
  ...
  // ── One-shot: load every student's current-lesson data + agility times ──
  function fetchAllCurrent() {
    return fetch(APPS_SCRIPT_URL + '?action=getAllCurrent&class=' + encodeURIComponent(CLASS_NAME))
      .then(function(r) { return r.json(); })
      .then(function(json) {
        if (json.error) throw new Error(json.error);
        currentLesson = parseInt(json.currentLesson, 10) || 1;
        if (json.students) {
          for (var name in json.students) {
            var ratings = blankRatings();
            var src = json.students[name];
            for (var k in src) { if (src[k] !== undefined) ratings[k] = src[k]; }
