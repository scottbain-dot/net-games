...
| **L1–L2** | Intro and the three skill tests in pairs, same set-up each time, a partner counts, scores written on the paper log. No laptops. | Marks the feed spots and zones before the lesson. Watches, spot-checks counts. |
| **L3 · Early check-in** (start of the lesson, laptops, ~8 min) | Types the three scores from paper, picks **one** focus skill (the lowest is recommended), edits the drafted goal, answers one question | Same day, phone or laptop, ~3 min: **Check-ins → Early**. Glance at each row, fix a wrong score, tap ✓, rate personal skills (one tap: Not yet / Sometimes / Consistently). |
| **L3–L5** | Works the drill steps on the paper log; a partner initials each step; you sign it | Circulates; **signs the step** to let a student move on. **No app.** |
| **Middle check-in** (L6, laptops) | Retests the focus skill (partner counts), types the score, taps the drill step reached, places themselves, one reflection. **Finished every step?** Chooses an extension skill and drill. | **Check-ins → Middle**: check the retest, ✓, personal skills. |
| **L7–L9** | Paper only. Last two lessons: game play. | Paper only. In the **last two lessons** open **Check-ins → Game play**: one row per student with their focus skill and four big buttons (7 · 6–5 · 4–3 · 2–1) plus an optional one-line note. Tap as you watch; it saves itself. |
| **End check-in** (L10) | Taps drill step, places themselves on the focus skill, personal-skill self-rating, two reflections | **Check-ins → End** is your final assessment: type the final retest, ✓, personal skills. The game-play level you tapped is shown on the row (tap to change it). |
...
6. Fill in the **Roster** tab: `Section`, `Sport`, `Student`, `Email` (school Google address). One row per student, all sections on the one tab. Section can be a plain number such as `7` or `8`; three classes that share a section simply share the value. Two students with the same name in one section would share a record, so give one of them a surname or initial (the config check warns you). The sport must match a name on the **Skills** tab exactly. Extra columns (e.g. a Teacher column for your own reference) are ignored.
...
- **Skills** — `Sport`, `Skill`, `Test` (how the test runs), `Success` (what counts as one), `Scale` (optional: for a timed or counted test, how the raw result converts to a score out of 10; prints under the test on the paper log and in the check-in form). **Three per sport**, one of them a stretch test that challenges a proficient player. Tests must be countable by a partner.
...
- **Lessons** — `Number`, `Checkpoint` (write `Early`, `Middle`, `End` on the check-in lessons; the draft has ten lessons with Early on lesson 3 so lessons 1 and 2 are intro and tests, Middle on 6, End on 10), `Date` (optional, prints on the log and drives the "Today · lesson N" strip at the top of the teacher view; without dates the strip shows the next check-in that still has rows to check). No titles: what happens in a lesson is up to you.
- **Outcomes** — personal skills for an independent task: self-management, perseverance, collaboration. `Outcome`, `LooksLike`. They print on the paper log as "what your teacher looks for"; students self-rate at the End check-in and you give one rating per check-in (tap *rate each* on a row for per-outcome detail).
- **BackPage** — optional page 2 of every printed student log, so a teacher can add their own material without touching the app. One row per line, in order: `Sport` (blank = every sport), `Line`. A line starting `# ` prints as a heading, `## ` a sub-heading, `- ` a bullet, `___` a ruled writing line, an empty line a gap; lines starting `|` with cells separated by `|` print as a table (first such line is the header). The draft ships the tab empty; leave a sport's rows empty for a single-sided log.
